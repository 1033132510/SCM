...
StringBuilder.prototype = function () {
    ...
    return {
        constructor: StringBuilder, length: 0, append: Array.prototype.push, appendFormat: function (e) {
    ...
    a.fn.bsgrid_paging = {
        defaults: {
            ...
            if (c <= 1) {
                zcal({
                    type: 'warning',
                    title: a.bsgridLanguage.isFirstPage,
                    text:'点击关闭进行其他操作'
                })
            ...
            if (c >= b.totalPages) {
                if (b.settings.loopback && c > 0) {
                    ...
                    zcal({
                        type: 'warning',
                        title: a.bsgridLanguage.isLastPage,
                        text:'点击关闭进行其他操作'
                    });
            ...
            if (a.trim(c) == "" || isNaN(c)) {
                zcal({
                    type: 'warning',
                    title: a.bsgridLanguage.needInteger,
                    text:'点击关闭进行其他操作'
                })
                ...
                if (parseInt(c) < 1 || parseInt(c) > b.totalPages) {
                    ...
                    zcal({
                        type: 'warning',
                        title: a.bsgridLanguage.needRange(1, b.totalPages),
                        text:'点击关闭进行其他操作'
                    });
                } else {
...
(function ($) {
    $.fn.bsgrid = {
        ...
        defaults: {
            ...
            requestParamsName: {pageSize: "pageSize", curPage: "curPage", sortName: "sortName", sortOrder: "sortOrder"},
            ...
            event: {selectRowEvent: false, unselectRowEvent: false, customRowEvents: {}, customCellEvents: {}},
            extend: {
                initGridMethods: {},
        ...
        init: function (gridId, settings) {
            ...
            var gridObj = {
                options: options, getPageCondition: function (curPage) {
...
(function (a) {
    a.fn.bsgrid_form = {
        defaults: {}, formObjs: {}, init: function (e, d) {
            var c = {
            ...
            var b = {
                options: c, addAssistShowFormTags: function () {
            ...
            if (c.startWith("view")) {
                a(".formInput :input:not(:button,:submit,:reset)", b.jqueryObj).css({"border-width": "0"}).attr("readOnly", "readOnly")
            } else {
                if (c.startWith("add")) {
                    a(".formInput :input:not(:button,:submit,:reset)", b.jqueryObj).css({border: "solid 1px #abadb3"}).removeAttr("readOnly")
                } else {
                    if (c.startWith("edit")) {
                        a(".formInput :input:not(:button,:submit,:reset)", b.jqueryObj).css({border: "solid 1px #abadb3"}).removeAttr("readOnly");
