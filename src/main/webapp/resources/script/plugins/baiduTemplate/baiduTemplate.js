...
(function(window) {
	...
	baidu.template = function(str, data) {
		...
		var fn = (function() {
			...
			if (element) {
				...
				var html = /^(textarea|input)$/i.test(element.nodeName) ? element.value
						: element.innerHTML;
	...
	bt._analysisStr = function(str) {
		...
		// 对变量后面的分号做容错(包括转义模式 如<%:h=value%>) <%=value;%> 排除掉函数的情况 <%fun1();%>
		// 排除定义变量情况 <%var val='test';%>
		.replace(
				new RegExp("(" + _left + ":?[hvu]?[\\s]*?=[\\s]*?[^;|" + _right
						+ "]*?);[\\s]*?" + _right, "g"), "$1" + _right_)
