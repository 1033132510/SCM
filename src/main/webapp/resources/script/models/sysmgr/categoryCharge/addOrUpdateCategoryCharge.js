		...
		bindAddOrUpdateClick : function() {
			$('#addOrUpdate').bind('click', function() {
				...
				$.ajax({
					...
					data : {ids : secondLevelCateIds.join(','), type : CATEGORY_ADMINISTRATOR, employeeId : employeeId},
					async : false,
				...
				$.ajax({
					...
					data : {categoryChargeVOList : JSON.stringify(categoryChargeList), employeeId : employeeId, type : CATEGORY_ADMINISTRATOR},
					type : 'post',
					success : function(data) {
						if(data.success) {
							zcal({
							...
							setTimeout(function() {
								location.href = ctx + '/sysmgr/category/charge/index';
							}, 3000);
						}
					},
					error : function() {
		...
		bindKeywordsInputEvent : function() {
			...
			$('#keywords').bind('focus', function() {
				...
				$.ajax({
					...
					data : {employeeName : keywords, type : CATEGORY_ADMINISTRATOR, limit : 5},
					type : 'post',
					success : function(data) {
						var i = 0, length,
						html = '', list = data;
