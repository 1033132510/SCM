...
function loadDetail(id, withVars, callback) {
    var dialog = this;
    $.getJSON(ctx + '/demo/leave/detail/' + id, function (data) {
        detail = data;
...
function loadDetailWithTaskVars(leaveId, taskId, callback) {
    var dialog = this;
    $.getJSON(ctx + '/demo/leave/detail-with-vars/' + leaveId + "/" + taskId, function (data) {
        detail = data;
        $.each(data, function (k, v) {
            // 格式化日期
            if (k == 'applyTime' || k == 'startTime' || k == 'endTime') {
...
function complete(taskId, variables) {
    ...
    /*$.blockUI({
     message: '<h2><img src="' + ctx + '/resources/content/demo/images/ajax/loading.gif" align="absmiddle"/>正在提交请求……</h2>'
     });*/

    // 发送任务完成请求
    $.post(ctx + '/demo/leave/complete/' + taskId, {
        keys: keys,
        values: values,
        types: types
    }, function (resp) {
    ...
    /*alert('t');
     $.getJSON(ctx + '/demo/leave/complete/' + taskId, function(data) {
     alert(data);
     });*/
}
...
function handle() {
    ...
    $('#' + tkey).data({
        ...
        open: function () {
            handleOpts[tkey].open.call(this, rowId, taskId, tkey);
        },
